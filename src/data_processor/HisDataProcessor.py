...
class HisDataProcessor(DataProcessor):
    ...
    def parse_dp_args(parser):
        ...
        parser.add_argument('--sup_his', type=int, default=1,
    ...
    def _get_feed_dict_rt(self, data, batch_start, batch_size, train):
        ...
        for c in self.data_columns:
            ...
            if c == global_p.C_HISTORY and self.sparse_his == 1:
                ...
                if c == 'history': #['X', 'history', 'history_pos_tag', 'history_length']
                    feed_dict[c] = utils.numpy_to_torch(np.stack(d))
                elif c == 'history_pos_tag':
                    feed_dict[c] = utils.numpy_to_torch(np.stack(d))
                else:
                    feed_dict[c] = utils.numpy_to_torch(d)
        return feed_dict
    ...
    def _get_feed_dict_rk(self, data, batch_start, batch_size, train, neg_data=None):
        if not train:
            ...
            for c in self.data_columns:
                # print(c)
                # print(data[c][batch_start: batch_start + real_batch_size].shape)
                # print(neg_columns_dict[c].shape)
                # if c == 'history':
                # d = np.concatenate([data[c][batch_start: batch_start + real_batch_size], neg_columns_dict[c].reshape(data[c][batch_start: batch_start + real_batch_size].shape)])
                d = np.concatenate([data[c][batch_start: batch_start + real_batch_size], neg_columns_dict[c]])
                # print(d.shape)
                if c == global_p.C_HISTORY and self.sparse_his == 1:
                # if c == global_p.C_HISTORY:
                    x, y = [], []
                    for idx, iids in enumerate(d):
                        x.extend([idx] * len(iids))
                        y.extend(iids)
    ...
    def format_data_dict(self, df):
        ...
        # his = history[global_p.C_HISTORY].apply(lambda x: eval('[' + x + ']'))
        # if self.max_his > 0:
        #     his = his.apply(lambda x: x[:self.max_his])
        #     if self.sup_his == 1:
        #         his = his.apply(lambda x: [-1] * (self.max_his - len(x)) + x)
        # data_dict[global_p.C_HISTORY] = his.values
        if self.max_his > 0 and self.sup_his == 1:
            ...
            # data_dict[global_p.C_HISTORY] = history[global_p.C_HISTORY]. \
            #     apply(lambda x: ([-1] * self.max_his + [int(i) for i in x.split(',')])[-self.max_his:] \
            #     if x != '' else np.array([])).values
